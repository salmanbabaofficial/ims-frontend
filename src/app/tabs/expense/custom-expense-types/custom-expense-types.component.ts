...
export class CustomExpenseTypesComponent {
  ...
  readonly deleteTarget = signal<ExpenseTypeRow | null>(null);
  readonly currentPage = signal(1);
  readonly pageSize = PAGE_SIZE;

  readonly rows = signal<ExpenseTypeRow[]>([
  ...
  openEdit(row: ExpenseTypeRow): void {
    this.editing.set(this.rowToFormValue(row));
    this.formMode.set('edit');
    this.mode.set('form');
  }

  openView(row: ExpenseTypeRow): void {
    this.editing.set(this.rowToFormValue(row));
  ...
  onSave(value: ExpenseTypeFormValue): void {
    ...
    if (this.formMode() === 'edit' && value.id != null) {
      ...
      const newRow: ExpenseTypeRow = this.mergeRow({ id: nextId } as ExpenseTypeRow, value);
  ...
  askDelete(row: ExpenseTypeRow): void {
  ...
  private rowToFormValue(row: ExpenseTypeRow): ExpenseTypeFormValue {
    return {
      id: row.id,
      name: row.name,
  ...
  private mergeRow(existing: ExpenseTypeRow, value: ExpenseTypeFormValue): ExpenseTypeRow {
    return {
      ...existing,
      name: value.name,
